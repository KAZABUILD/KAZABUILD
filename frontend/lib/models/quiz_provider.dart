...
class QuizAnswers {
  ...
  QuizAnswers copyWith({Map<String, QuizAnswerSelection>? selections}) {
    return QuizAnswers(selections: selections ?? this.selections);
  }
}
...
class QuizService {
  ...
  Future<List<QuizQuestion>> fetchQuestions() async {
    ...
    for (final dynamic pref in preferenceJson) {
      ...
      final parentAnswerId = _asString(
        pref['userPreferenceAnswerId'] ?? pref['UserPreferenceAnswerId'],
      );

      questions[id] = QuizQuestion(
        id: id,
        text: question,
        parentAnswerId: parentAnswerId != null && parentAnswerId.isNotEmpty
            ? parentAnswerId
            : null,
      );
    }
    ...
    for (final dynamic answer in answersJson) {
      ...
      final questionId = _asString(
        answer['userPreferenceId'] ?? answer['UserPreferenceId'],
      );
    ...
    for (final question in questions.values) {
      final options =
          (optionsByQuestion[question.id] ?? [])
              .map(
                (option) =>
                    option.copyWith(followUpQuestionId: followUpMap[option.id]),
              )
              .toList()
  ...
  Future<List<QuizAnswerSelection>> fetchUserSelections({
    ...
    for (final dynamic answer in answers) {
      if (answer is! Map<String, dynamic>) continue;
      final answerId = _asString(
        answer['userPreferenceAnswerId'] ?? answer['UserPreferenceAnswerId'],
      );
  ...
  Future<String> submitAnswer({
    ...
    final response = await _dio.post(
      ...
      data: {'UserId': userId, 'UserPreferenceAnswerId': answerId},
    );
  ...
  Future<List<build_model.Build>> fetchGeneratedBuilds({
    required String userId,
  }) async {
    final response = await _dio.post(
      '$apiBaseUrl/Builds/get',
