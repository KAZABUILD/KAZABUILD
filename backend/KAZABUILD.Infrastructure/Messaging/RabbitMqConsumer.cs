...
{
    public class RabbitMQConsumer(IRabbitMqConnection connection, IOptions<RabbitMQSettings> settings) : BackgroundService
    {
        ...
        {
            //Get the connection
            var connection = await _connection.GetConnectionAsync().ConfigureAwait(false);
